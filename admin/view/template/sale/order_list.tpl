...
<!-- modal -->
<div id="order-products-modal" title="Order Products" style="display:none;">
    ...
    <div id="modal-loading" style="display:none; text-align:center; margin-top:10px;">Loading...</div>
</div>

<script>
$(document).ready(function() {

    $('.view-products-button').on('click', function(e) {
        e.preventDefault();

        var order_id = $(this).data('order-id');
        var token = '<?php echo $token; ?>';
        // Clear previous rows
        $('#modal-products-body').html('');
        $('#modal-loading').show();

        $('#order-products-modal').dialog({
            modal: true,
            width: 700,
            title: 'Order #' + order_id + ' Products'
        });

        $.ajax({
            url: 'index.php?route=sale/order/products&token=' + token + '&order_id=' + order_id,
            type: 'GET',
            dataType: 'json',
            success: function(data) {
                ...
                if (data.length > 0) {
                    ...
                    $('#modal-products-body').html('<tr><td colspan="5" style="text-align:center;">No products found</td></tr>');
                }
            },
            error: function(xhr, status, error) {
                $('#modal-loading').hide();
                $('#modal-products-body').html('<tr><td colspan="5" style="color:red; text-align:center;">Error loading products</td></tr>');
