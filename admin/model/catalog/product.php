...
class ModelCatalogProduct extends Model
{
	...
	public function addCategoryWithParents($product_id, $category_id)
	{
		...
		// If parent category exists, recursively add it
		if ($query->row && (int)$query->row['parent_id'] > 0) {
			$this->addCategoryWithParents($product_id, (int)$query->row['parent_id']);
		}
	}
	...
	public function getCategoryFullPath($category_id)
	{
		...
		while ($category_id > 0) {
			...
			if ($query->row) {
				array_unshift($paths, $query->row['name']);
	...
	public function addProduct($data)
	{
		...
		if (isset($data['product_category'])) {
			foreach ($data['product_category'] as $category_id) {
				$this->addCategoryWithParents($product_id, $category_id);
			}
		}
	...
	{
		$sql = "SELECT p.*, pd.*, (p.price - p.cost) AS profit
		...
		// Custom cost filter supporting inequalities: [ < , > , <= , >= , != ]
		if (!empty($data['filter_cost'])) {
			$cost_filter = $data['filter_cost'];
			$filter_operator = $data['filter_operator'];
			$filter_value = (float)$data['filter_value'];
			$sql .= " AND CAST(p.cost AS DECIMAL(10.3))" . $filter_operator . " '" . $filter_value . "'";
		}


		if (!empty($data['filter_profit'])) {
			$filter_profit_operator = $data['filter_profit_operator'];
			$filter_profit_value = (float)$data['filter_profit_value'];
			$sql .= " AND (p.price - p.cost)" . $filter_profit_operator . " '" . $filter_profit_value . "'";
		}
	...
	public function getTotalProducts($data = array())
	{
		...
		if (!empty($data['filter_profit'])) {
			$sql .= " AND (p.price - p.cost) LIKE '" . $this->db->escape($data['filter_profit']) . "%'";
		}
		...
		if (isset($data['Filter_category_id']) && !is_null($data['Filter_category_id'])) {
			$sql .= " AND product_to_category.category_id = '" . (int)$data['Filter_category_id'] . "'";
		}
