...
class ControllerCatalogCategory extends Controller
{
	...
	protected function getList()
	{
		...
		if (isset($this->request->get['filter_product_count'])) {
			$filter_product_count = trim($this->request->get['filter_product_count']);
		} else {

			$filter_product_count  = null;
		}

		if ($filter_product_count !== null && $filter_product_count !== '') {
			$url .= '&filter_product_count=' . urlencode($filter_product_count);
		}
		...
		foreach ($results as $result) {
			...
			$productPerCategory = (int)$this->model_catalog_category->getProductCountByCategory($result['category_id']);

			if ($filter_product_count !== null && $filter_product_count !== '') {
				$filter_product_count = trim(html_entity_decode($filter_product_count));
				if (preg_match('/^(>=|<=|>|<|=|!=)?\s*(\d+(\.\d+)?)$/', $filter_product_count, $matches)) {
					$filter_count_operator = $matches[1] ?: '=';
					$filter_count_value = (int)$matches[2];
					$show = false;
					switch ($filter_count_operator) {
						case '>':
							$show = $productPerCategory > $filter_count_value;
							break;
						case '<':
							$show = $productPerCategory < $filter_count_value;
							break;
			...
			$categories_parent = '';
			$categories_parent = $this->model_catalog_category->getParentCategory($result['category_id']);
		...
		$this->data['categories_parent'] = $categories_parent;
		...
		if ($sort == 'product_count') {
			usort($this->data['categories'], function ($a, $b) use ($order) {
				if ($a['productPerCategory'] == $b['productPerCategory']) {
					return 0;
				}

				if ($order == 'ASC') {
					return ($a['productPerCategory'] < $b['productPerCategory']) ? -1 : 1;
				} else {
					return ($a['productPerCategory'] > $b['productPerCategory']) ? -1 : 1;
				}
			});
