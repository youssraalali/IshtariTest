...
class ControllerCatalogProduct extends Controller
{
	...
	protected function getList()
	{
		...
		if (isset($this->request->get['filter_profit'])) {
			$filter_profit = trim($this->request->get['filter_profit']);
		} else {
			$filter_profit = null;
		}

		if (isset($this->request->get['filter_cost'])) {
			$filter_cost = trim($this->request->get['filter_cost']);
		} else {
			$filter_cost = null;
		}
		...
		// Parse filter_cost for operator and value
		$filter_operator = '';
		$filter_value = '';

			if ($filter_cost !== null && $filter_cost !== '') {
			$filter_cost = trim(html_entity_decode($filter_cost));
			if (preg_match('/^(>=|<=|>|<|=|<>|!=)?\s*(\d+(\.\d+)?)$/', $filter_cost, $matches)) {
				$filter_operator = $matches[1] ?: '=';
		...
		//Parse filter_profit for operator and value 

		$filter_profit_operator = '';
		$filter_profit_value = '';

			if ($filter_profit !== null && $filter_profit !== '') {
			$filter_profit = trim(html_entity_decode($filter_profit));
			if (preg_match('/^(>=|<=|>|<|=|<>|!=)?\s*(\d+(\.\d+)?)$/', $filter_profit, $matches)) {
				$filter_profit_operator = $matches[1] ?: '=';
				$filter_profit_value = $matches[2];
			}
		}

		$data['filter_profit_operator'] = $filter_profit_operator;
		$data['filter_profit_value'] = $filter_profit_value;
		...
		if ($filter_cost !== null && $filter_cost !== '') {
			$url .= '&filter_cost=' . urlencode($filter_cost);
		}

		if ($filter_profit !== null && $filter_profit !== '') {
			$url .= '&filter_profit=' . urlencode($filter_profit);
		}
		...
		$data = array(
			...
			'filter_profit_operator' => $filter_profit_operator,
			'filter_profit_value'   => $filter_profit_value,
		...
		foreach ($results as $result) {
			...
			foreach ($product_categories as $category_id) {
				$category_full_path[] = $this->model_catalog_product->getCategoryFullPath($category_id);
			}

			$categories_id = $this->model_catalog_product->getCategoryId();
			$categories_paths = [];
			foreach($categories_id as $category_id) {
				$categories_paths []  = [
					'category_id' => $category_id,
					'path' => $this->model_catalog_product->getCategoryFullPath($category_id)
				];
			}
			$this->data['categories_paths'] = $categories_paths;
